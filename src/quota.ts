/**
 * Quota data collection for third-party model providers.
 *
 ...
 *
 * Key design points:
 *  - `collectQuota` NEVER throws: failures degrade to fewer/missing segments.
...
import type { QuotaConfig, QuotaProviderConfig } from './config.js';
 ...
 */
export function resolveQuotaModel(
  stdin: StdinData,
 ...
 */
export function matchProviders(quotaCfg: QuotaConfig, model: string): QuotaProviderId[] {
  const withKey = Object.entries(quotaCfg.providers)
    .filter(([, p]) => p.apiKey)
    .map(([name]) => name) as QuotaProviderId[];
  if (quotaCfg.displayMode === 'all') return withKey;
  const low = model.toLowerCase();
  for (const name of withKey) {
    const p = quotaCfg.providers[name];
...
function windowFrom(o: Record<string, unknown>, label: QuotaWindow['label']): QuotaWindow | null {
...
export function parseKimiResponse(raw: unknown): QuotaData | null {
...
export function parseDeepSeekResponse(raw: unknown): QuotaData | null {
  if (!raw || typeof raw !== 'object') return null;
 ...
 */
export function parseGlmResponse(raw: unknown): QuotaData | null {
  if (!raw || typeof raw !== 'object') return null;
...
async function fetchKimi(key: string, fetchImpl: typeof fetch): Promise<QuotaData> {
...
async function fetchDeepSeek(key: string, fetchImpl: typeof fetch): Promise<QuotaData> {
  const raw = await getJson(
...
async function fetchGlm(key: string, endpoint: string | undefined, fetchImpl: typeof fetch): Promise<QuotaData> {
...
function reviveQuotaData(data: QuotaData): QuotaData {
...
function writeCache(cacheDir: string, provider: string, keyHash: string, updatedAt: number, data: QuotaData): void {
  try {
...
async function fetchOrCache(provider: QuotaProviderId, cfg: QuotaProviderConfig, deps: QuotaDeps): Promise<QuotaData> {
  ...
  try {
    ...
    throw new Error(`quota fetch failed: ${provider}`);
  }
}
...
export async function collectQuota(
  ...
  stdin: StdinData,
  transcript: TranscriptData,
  deps?: Partial<QuotaDeps>,
): Promise<QuotaData[]> {
  const d: QuotaDeps = {
    fetchImpl: globalThis.fetch,
    now: () => Date.now(),
    cacheDir: () => path.join(getHudPluginDir(os.homedir()), 'quota-cache'),
    ...deps,
  };
  try {
    const quotaCfg = config.quota;
    if (!quotaCfg?.enabled) return [];
    const model = resolveQuotaModel(stdin, transcript, config.display?.modelSource ?? 'auto');
    const providers = matchProviders(quotaCfg, model);
    if (providers.length === 0) return [];
    const settled = await Promise.allSettled(providers.map(p => fetchOrCache(p, quotaCfg.providers[p], d)));
