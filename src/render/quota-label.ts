 ...
 *   🟢 < 60%  🟡 60–84%  🔴 ≥ 85%
 */
import type { QuotaProviderId, QuotaData } from '../quota.js';
import type { MessageKey } from '../i18n/types.js';

const PROVIDER_LABELS: Record<QuotaProviderId, string> = { kimi: 'Kimi', deepseek: 'DS', glm: 'GLM' };
...
function windowsLabel(data: QuotaData, t: (k: MessageKey) => string): string {
  const windows = data.windows ?? [];
  if (windows.length === 0) return '';
  const max = Math.max(...windows.map(w => w.percent));
  const parts = windows.map(w => `${w.label === 'week' ? t('label.quotaWeek') : w.label} ${w.percent}%${fmtReset(w.resetAt)}`);
  return `${emojiOf(max)} ${PROVIDER_LABELS[data.provider]} ${parts.join(' · ')}`;
}

function balanceLabel(data: QuotaData): string {
...
export function formatQuotaLabel(data: QuotaData[], t: (k: MessageKey) => string): string | null {
  const parts: string[] = [];
  for (const d of data) {
