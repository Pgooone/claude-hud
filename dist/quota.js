 ...
 */
export function resolveQuotaModel(stdin, transcript, modelSource) {
 ...
 */
export function matchProviders(quotaCfg, model) {
    const withKey = Object.entries(quotaCfg.providers)
        .filter(([, p]) => p.apiKey)
        .map(([name]) => name);
    if (quotaCfg.displayMode === 'all')
        return withKey;
...
export async function collectQuota(config, stdin, transcript, deps) {
    const d = {
